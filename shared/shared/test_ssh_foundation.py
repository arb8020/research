...
def test_ssh_connection_info():
    ...
    conn = SSHConnectionInfo(hostname="1.2.3.4", port=22, username="root", timeout=30)
...
def test_universal_ssh_client_structure():
    ...
    for method in sync_methods:
        ...
        assert not asyncio.iscoroutinefunction(getattr(client, method)), (
...
def test_convenience_functions():
    ...
    assert asyncio.iscoroutinefunction(execute_command_async), (
        "execute_command_async should be async"
    )
