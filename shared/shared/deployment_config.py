...
class GPUConfig:
    ...
    def __post_init__(self):
        ...
        assert all(r >= 0 for r in self.gpu_ranks), f"gpu_ranks must be non-negative: {self.gpu_ranks}"
        assert len(self.gpu_ranks) == len(set(self.gpu_ranks)), f"gpu_ranks has duplicates: {self.gpu_ranks}"
