...
def validate_credentials(credentials: dict[str, str]) -> dict[str, str]:
    ...
    valid_providers = {"runpod", "primeintellect", "lambdalabs", "vast", "digitalocean"}
...
def validate_gpu_count(gpu_count: int) -> int:
    ...
    assert 1 <= gpu_count <= 8, f"gpu_count must be 1-8, got {gpu_count}"
...
def validate_price(price: float, max_price: float = 10.0) -> float:
    ...
    assert price <= max_price, f"price exceeds maximum ${max_price}/hr, got ${price}/hr"
...
def validate_instance_name(name: str) -> str:
    ...
    assert all(c in valid_chars for c in name), f"name contains invalid characters: {name}"
