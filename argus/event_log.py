...
class JsonlEventSink:
    """Append-only JSONL sink for journal events."""
...
def journal_path(output_dir: str | Path, journal_name: str) -> Path:
    """Resolve one journal path under a run directory."""
    return Path(output_dir) / journal_name


def list_jsonl_journal_paths(run_dir: str | Path) -> tuple[Path, ...]:
    """List JSONL journals under a run directory in stable name order."""
    return tuple(sorted(Path(run_dir).glob("*.jsonl")))


def existing_journal_paths(run_dir: str | Path, journal_names: Iterable[str]) -> tuple[Path, ...]:
    """Return the subset of requested journals that currently exist."""
    base_dir = Path(run_dir)
    paths: list[Path] = []
    for journal_name in journal_names:
        path = base_dir / journal_name
        if path.exists():
            paths.append(path)
    return tuple(paths)
