...
def test_runpod_custom_images_get_docker_args() -> None:
    script = argus_run._runpod_custom_image_docker_args("slimerl/slime:v0.2.3")

    assert script is not None
    assert "openssh-server" in script
    assert script == argus_run.RUNPOD_DOCS_CUSTOM_IMAGE_SSH_DOCKER_ARGS
    assert "service ssh start" in script
    assert "sleep infinity" in script


def test_runpod_official_images_do_not_get_docker_args() -> None:
    script = argus_run._runpod_custom_image_docker_args(
