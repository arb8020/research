"""Training script for iGSM with retry data.

Adapted from train_igsm.py to use retry data loader.
...
def train(
    ...
    """Main training function for iGSM with retry data.

    Args:
        config: Training configuration
        igsm_config: iGSM config including retry_rate and retry_type
    ...
    # Create RETRY data loaders
    retry_rate = igsm_config.get("retry_rate", 0.1)
    retry_type: Literal["strong", "weak"] = igsm_config.get("retry_type", "strong")

    train_loader = build_igsm_retry_loader(
    ...
    val_loader = build_igsm_retry_loader(
        difficulty=igsm_config.get("difficulty", "med"),
        retry_rate=retry_rate,
        retry_type=retry_type,
        seq_len=config.max_seq_len,
    ...
    if is_main():
        logger.info(f"Using RETRY data loader: rate={retry_rate}, type={retry_type}")
