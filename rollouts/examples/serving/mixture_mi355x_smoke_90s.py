...
Usage:
    python -m argus run \\
        --config rollouts/examples/serving/mixture_mi355x_smoke_90s.py \\
...
serving_scenario = ServingScenario(
    endpoint=endpoint,
    hardware=hardware,
    # Large max_samples so natural exhaustion doesn't short-circuit the
    # duration watchdog — we want the 90s timer to be what triggers
    # shutdown, so we actually exercise drain.
    workloads=[
        EvalServingWorkload(
            name="tau2_retail",
    ...
    output=ServingOutputConfig(
        experiment_name="mixture_mi355x_smoke_90s",
    ),
    ...
    # Exercise the duration watchdog: after 90s, shutdown_event fires,
    # supervisor SIGINTs the eval child for a clean drain, reports written.
    duration=timedelta(seconds=90),
    # Give in-flight tau2 turns a bit of headroom to finish cleanly before
    # we escalate to SIGTERM/SIGKILL.
    drain_timeout=timedelta(seconds=60),
)
