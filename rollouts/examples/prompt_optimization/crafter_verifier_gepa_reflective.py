"""Reflective GEPA optimization for a Crafter game verifier.
...
When the verifier predicts the wrong score, the mutation LLM sees:
- The trajectory that was evaluated
- The predicted score vs the human score
- Specific feedback about what the verifier missed

This targeted feedback helps the optimizer learn domain-specific scoring criteria.
For comparison, see crafter_verifier_gepa_evolutionary.py which uses blind mutations.
...
Run with:
    python -m examples.prompt_optimization.crafter_verifier_gepa_reflective
"""
...
async def main() -> None:
    ...
    # ─── Initial Verifier Prompt ──────────────────────────────────────────
    initial_system = """You are a game trajectory evaluator for Crafter, a survival/crafting game.
...
Score: X.XX"""
    ...
    # ─── Run Reflective GEPA ──────────────────────────────────────────────
    logger.info("Starting reflective GEPA optimization for Crafter Verifier...")
...
Provide your score (0.0 to 1.0):""",
    ...
    logger.info(f"Best accuracy: {result.best_score:.3f}")
    ...
    logger.info(f"within {(1 - result.best_score):.1%} of human scores.")
    logger.info("")

    if result.history:
        logger.info("Progress:")
        for h in result.history[-5:]:
            logger.info(f"  iter={h['iteration']}, best={h['best_score']:.3f}")
