...
def make_simple_dataset(n_samples: int):
...
def simple_score_fn(sample):
    ...
    return Score(
        metrics=(
            Metric(name="correct", value=1.0 if correct else 0.0, weight=1.0),
        ),
    )
...
async def run_stress_test(
    ...
    def prepare_messages(sample: dict) -> list[Message]:
        return [
            Message(role="system", content="You are a helpful math assistant. Answer with just the number."),
            Message(role="user", content=sample["question"]),
        ]
    
    # Create RunConfig with stop condition - single turn only!
    async def noop_on_chunk(event):
        pass
    ...
    logger.info(f"  (progress bar should appear below - each sample takes ~2-5s)")
    ...
    provider_errors = sum(
        1 for s in report.sample_results 
        if s.metadata.get("error_type") == "provider_error"
    )
    other_errors = sum(
        1 for s in report.sample_results 
