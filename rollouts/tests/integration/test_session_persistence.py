...
import rollouts.agents as agents
import rollouts.agents.runtime as agent_runtime
from rollouts.agents import Actor, AgentState, RunConfig, StopReason, resume_session, run_agent
...
@dataclass
class ScriptedCalculatorEnvironment:
    ...
    @staticmethod
    async def deserialize(data: dict) -> "ScriptedCalculatorEnvironment":
        return ScriptedCalculatorEnvironment(
...
async def run_with_script(
    ...
    """
    from rollouts.core import TextContent, ToolCallContent

    llm = ScriptedLLM(script)
    original_rollout = agents.rollout
    original_runtime_rollout = agent_runtime.rollout
    ...
    agent_runtime.rollout = mock_rollout
    try:
        return await run_agent(initial_state, run_config)
    finally:
        agents.rollout = original_rollout
        agent_runtime.rollout = original_runtime_rollout
...
async def test_session_persistence_with_model_swap() -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        env1 = ScriptedCalculatorEnvironment(current_value=0.0)
        ...
        env2 = ScriptedCalculatorEnvironment(
            current_value=15.0,
        ...
        env3 = ScriptedCalculatorEnvironment(
            current_value=25.0,
