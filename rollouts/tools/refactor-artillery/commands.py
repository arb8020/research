...
def parse_commands(response: str) -> list[Command]:
    ...
    write_pattern = re.compile(r'<write\s+file=["\']([^"\']+)["\']>(.*?)</write>', re.DOTALL)
    ...
    for match in patch_pattern.finditer(response):
        ...
        search_replace = re.search(r"<{7}\s*SEARCH\s*\n(.*?)\n={7}\n(.*?)\n>{7}", inner, re.DOTALL)
        if search_replace:
            search = search_replace.group(1)
            replace = search_replace.group(2)
            commands.append(
                PatchCommand(file=file_path, block_id=None, search=search, replace=replace)
            )

    # Parse <patch id=N>content</patch> (block-based)
    block_patch_pattern = re.compile(r"<patch\s+id=!?(\d+)>(.*?)</patch>", re.DOTALL)
...
def apply_commands(
    ...
    block_state: BlockState object for block-based patching.
    """
    ...
    for cmd in commands:
        if isinstance(cmd, WriteCommand):
            ...
            result = _apply_patch(cmd, workspace_root, block_state)
...
def _apply_write(cmd: WriteCommand, workspace_root: Path) -> ApplyResult:
    ...
    try:
        ...
        return ApplyResult(success=False, message=f"Path {cmd.file} is outside workspace")
...
def _apply_patch(
    ...
    if cmd.block_id is not None:
        if block_state is None:
            return ApplyResult(
                success=False, message=f"Block {cmd.block_id} patch requires block_state"
            )

        success, message = apply_block_patch(block_state, cmd.block_id, cmd.replace, workspace_root)
        return ApplyResult(success=success, message=message)
    ...
    if cmd.search is not None:
        ...
        if cmd.search not in content:
            return ApplyResult(success=False, message=f"Search pattern not found in {cmd.file}")
