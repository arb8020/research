...
def check_current_auth():
    ...
    if api_key_env:
        ...
        print(f"   ⚠️  Used as fallback if OAuth not available")
    else:
    ...
    try:
        ...
        if tokens:
            ...
            if tokens.is_expired():
                print(f"   ⚠️  Token expired (will auto-refresh on use)")
            else:
                print(f"   ✅ Token valid")
                
            if tokens.access_token.startswith("sk-ant-oat01-"):
                print(f"   ✅ Correct OAuth format")
            else:
                print(f"   ❌ Wrong token format!")
        else:
    ...
    if endpoint.oauth_token and endpoint.oauth_token.startswith("sk-ant-oat01-"):
        ...
        print(f"  • Billing: Anthropic API account")
        print(f"\n💡 To use OAuth instead:")
        if api_key_env:
            print(f"  1. unset ANTHROPIC_API_KEY")
        print(f"  2. Ensure OAuth login: python -m rollouts.cli --login-claude")
        return False
    else:
...
def test_api_key_path():
    ...
    import trio
    from rollouts.dtypes import Endpoint, Actor, Trajectory, Message, StreamEvent
    ...
    async def test():
        ...
        print(f"Expected: 401 authentication error\n")
        
        actor = Actor(
            trajectory=Trajectory(messages=[Message(role="user", content="hi")]),
            endpoint=endpoint
        )
        ...
        try:
            ...
            if "401" in str(e) or "authentication" in str(e).lower():
                print(f"✅ Got expected auth error:")
                print(f"   {str(e)[:100]}...")
                print(f"\n✅ This proves API key is used when set")
                print(f"💡 If the key were valid, you'd get invoiced")
                return True
            else:
...
def test_cli_output():
    ...
    print(f"Looking for: '🔐 Using OAuth authentication'\n")
    ...
    try:
        ...
        if "🔐 Using OAuth authentication" in output:
            print(f"✅ OAuth message found!")
            print(f"✅ CLI is using OAuth")
        else:
            print(f"⚠️  No OAuth message found")
        ...
        if "🔐 Using OAuth authentication" in output:
            print(f"✅ OAuth message found in partial output")
            print(f"⏱️  Timed out during API call (expected)")
        else:
            print(f"⚠️  Timed out before seeing OAuth message")
            print(f"Output preview: {output[:300]}")
    except Exception as e:
        print(f"❌ Error: {e}")
