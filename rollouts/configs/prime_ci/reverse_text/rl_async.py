"""Prime-CI reverse-text RL config using the conservative async pipeline."""
...
config_status = import_tested(
    "70bce1bf",
    "Imports cleanly and covers the conservative async GRPO path.",
)

config = GRPOConfig(
    output=GRPOOutputConfig(experiment_name="prime_ci_reverse_text_async"),
    model=ModelConfig(
...
def train(config: GRPOConfig = config, max_samples: int = 128, **kwargs: object) -> dict:
