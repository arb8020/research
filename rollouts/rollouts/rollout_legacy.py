...
class Message:
    ...
    role: str
    content: str | List[Dict[str, Any]]  # List[Dict] for vision (brittle - no schema enforcement)
...
class Endpoint:
    ...
    max_tokens: int = 2048
    max_retries: int = 3  # Explicit retry count
    backoff_base_seconds: float = 1.0  # Base delay for exponential backoff
    timeout_seconds: float = 120.0  # Request timeout (vision models need longer than default 60s)


def _is_retryable_error(error: str) -> bool:
    """Determine if error is worth retrying.

    Sean Goedecke principle: Don't blindly retry - think about failure modes.
    Context length errors and auth errors can't be fixed by retrying.
    ...
    Returns:
        ...
        False if error is non-retryable (context length, auth, invalid request)
    """
    # Non-retryable errors (explicit list)
    ...
    # Everything else is retryable (network errors, rate limits, timeouts, 5xx)
    return True
...
async def _generate_single_attempt(
    ...
    try:
        ...
        client = AsyncOpenAI(
            ...
            max_retries=0,  # Explicit: we handle retries ourselves
            timeout=httpx.Timeout(endpoint.timeout_seconds, connect=10.0)  # Timeout for vision models
        )
        ...
        return (new_rollout, None)

    except Exception as e:
        # Convert exception to error string (tuple return pattern)
...
async def generate(
    ...
    """Generate with retries, returns tuple (result, error).

    Tiger Style: Tuple returns are the default for explicit error handling.
    Casey Muratori: This is the main API - use generate_exception() if you want raises.
    ...
    Returns:
        (rollout, None) on success
        (None, error_message) on failure after all retries
    """
    import logging
    import asyncio
    ...
    # Explicit retry loop with fixed upper bound (Sean Goedecke: put limits on everything)
    for attempt in range(1, endpoint.max_retries + 1):
        result, error = await _generate_single_attempt(endpoint, rollout, on_chunk)

        if result is not None:
            # Success - return immediately
            if attempt > 1:
                logger.info(f"Succeeded on attempt {attempt}/{endpoint.max_retries}")
            return (result, None)

        # Failed - check if retryable
        assert error is not None, "error must be set if result is None"
        is_retryable = _is_retryable_error(error)

        # Log all decisions (Sean Goedecke: log aggressively on unhappy paths)
        if attempt < endpoint.max_retries and is_retryable:
            backoff_seconds = endpoint.backoff_base_seconds * (2 ** (attempt - 1))
            logger.warning(
                f"Attempt {attempt}/{endpoint.max_retries} failed: {error}. "
                f"Retrying in {backoff_seconds:.1f}s"
            )
            await asyncio.sleep(backoff_seconds)
        else:
            # Final attempt or non-retryable error
            reason = "non-retryable error" if not is_retryable else "max retries exhausted"
            logger.error(
                f"Failed after {attempt} attempts ({reason}): {error}"
            )
            return (None, f"Failed after {attempt} attempts: {error}")

    # Should never reach here due to loop structure, but satisfy type checker
    return (None, "Unexpected: retry loop exited without result")
...
async def generate_exception(
    ...
    """Generate with retries (raises on failure after max_retries).

    Casey Muratori: Alternative API for simple cases that want exceptions.
    Tiger Style: Use generate() with tuple returns as the default.
    ...
    """
    result, error = await generate(endpoint, rollout, on_chunk)

    if error:
        raise Exception(error)

    assert result is not None, "result must be set if error is None"
    return result
