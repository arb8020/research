...
def convert_verifiers_dataset_to_rollouts(
    verifiers_env: VerifiersEnv,
    use_eval_dataset: bool = False,
) -> list[Dict[str, Any]]:
    """Convert Prime verifiers dataset to rollouts format.

    Args:
        verifiers_env: Prime environment with dataset
        use_eval_dataset: Use eval_dataset instead of train dataset

    Returns:
        List of sample dicts compatible with rollouts.evaluation

    Example:
        >>> from verifiers import SingleTurnEnv
        >>> env = SingleTurnEnv(dataset=ds)
        >>> samples = convert_verifiers_dataset_to_rollouts(env)
        >>> # Use with rollouts.evaluation.evaluate()
    """
    dataset = verifiers_env.eval_dataset if use_eval_dataset else verifiers_env.dataset

    if dataset is None:
        raise ValueError("verifiers_env has no dataset")

    samples = []
    for item in dataset:
        # Convert to rollouts format
        sample = {
            "question": item.get("prompt", item.get("question", "")),
            "answer": item.get("answer", ""),
            # Preserve all other fields
            **{k: v for k, v in item.items() if k not in ["prompt", "question", "answer"]},
        }
