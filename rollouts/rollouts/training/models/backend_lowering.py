...
def lower_model_to_megatron(
    ...
    *,
    ...
    if family in {"qwen3_5", "qwen3_5_moe"}:
        ...
        adapter_kind = "custom_spec"
        notes.append(
            "glm4 lowers through the bridge-native transformer layer spec. The legacy "
            "bridge_gpt_fallback path remains available for debugging, but current "
            "Megatron runtime behavior is closer to green on the bridge-native path."
        )
