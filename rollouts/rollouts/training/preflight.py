...
# Runtime compatibility preflights
# These are workload-owned checks for package/API compatibility in the target
# runtime. Argus may record the result, but the semantics live here.
...
class RuntimePreflightResult:
    ...
    def require_ok(self) -> None:
        ...
        detail_lines = [f"{key}={value}" for key, value in sorted(details.items())]
...
def preflight_torchtitan_runtime() -> RuntimePreflightResult:
    ...
    This should fail before any real training work starts and before any
    distributed init if the environment is incompatible.
    """
...
def preflight_torchtitan_vram_check(
    ...
    from .vram_pytorch_like import _compute_micro_batch_size
