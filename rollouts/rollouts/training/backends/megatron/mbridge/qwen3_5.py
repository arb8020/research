...
"""Custom mbridge bridge for Qwen3.5 models."""
...
class Qwen3_5Bridge(Qwen2MoEBridge):
    """Bridge for Qwen3.5 dense and MoE variants."""
    ...
    _MTP_MLP_MAPPING = {
        ...
        "mlp.experts.linear_fc2": [
            "mtp.layers.{layer_number}.mlp.experts.{expert_id}.down_proj.weight"
        ],
    }
    ...
    def _convert_mtp_param(self, name: str) -> list[str]:
        ...
        if "transformer_layer" in name:
            ...
            return [
                cn.replace(
                    f"model.language_model.layers.{mtp_layer_idx}",
                    f"mtp.layers.{mtp_layer_idx}",
                )
