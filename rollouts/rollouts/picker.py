...
def _print_menu(
    title: str, options: list[tuple[str, str, bool]], default_idx: int = 0
) -> int | None:
...
def pick_driver() -> DriverType | None:
    ...
    options = [(info["name"], info["description"], info["available"]) for info in DRIVERS.values()]
...
def pick_model_interactive(
    skip_driver: bool = False, driver: DriverType | None = None
) -> PickerResult | None:
