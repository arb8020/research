...
def get_transformers_from_local_checkpoint(
    input_ids_BL: np.ndarray, model_name: str = "Llama-3.2-1B-Instruct"
) -> np.ndarray:
    """
    ...
    checkpoint_path = Path(
        f"~/.llama/checkpoints/{local_model_name}/consolidated.00.pth"
    ).expanduser()
    ...
    for i in range(16):  # 16 layers for Llama-3.2-1B
        # Attention norms
        transformers_state_dict[f"model.layers.{i}.input_layernorm.weight"] = checkpoint[
            f"layers.{i}.attention_norm.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.post_attention_layernorm.weight"] = checkpoint[
            f"layers.{i}.ffn_norm.weight"
        ]

        # Attention weights (transpose back since entropix uses transposed format)
        transformers_state_dict[f"model.layers.{i}.self_attn.q_proj.weight"] = checkpoint[
            f"layers.{i}.attention.wq.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.self_attn.k_proj.weight"] = checkpoint[
            f"layers.{i}.attention.wk.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.self_attn.v_proj.weight"] = checkpoint[
            f"layers.{i}.attention.wv.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.self_attn.o_proj.weight"] = checkpoint[
            f"layers.{i}.attention.wo.weight"
        ]

        # FFN weights
        transformers_state_dict[f"model.layers.{i}.mlp.gate_proj.weight"] = checkpoint[
            f"layers.{i}.feed_forward.w1.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.mlp.up_proj.weight"] = checkpoint[
            f"layers.{i}.feed_forward.w3.weight"
        ]
        transformers_state_dict[f"model.layers.{i}.mlp.down_proj.weight"] = checkpoint[
...
def get_local_pytorch_llama_logits(
    input_ids_BL: np.ndarray, model_name: str = "Llama-3.2-1B-Instruct"
) -> np.ndarray:
    """
