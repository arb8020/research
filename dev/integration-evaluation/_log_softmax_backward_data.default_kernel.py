...
def _log_softmax_backward_data__default_kernel_impl(*args, **kwargs):
    ...
    if len(args) >= 2:
        ...
        if grad_output is None or output is None:
            raise ValueError(
                "grad_output and output tensors must be provided as positional or keyword arguments"
            )
    ...
    if grad_output.shape != output.shape:
        raise ValueError(
            f"grad_output and output must have the same shape, got {grad_output.shape} and {output.shape}"
        )
