    ...
    for layer in range(num_layers):
        ...
        if num_experts and num_experts > 1:
            ...
            if layer == 0:  # Only show breakdown for first layer
                ...
                breakdown.append(
                    f"Layer (MoE): {layer_params / 1e9:.1f}B ({num_experts} experts, {active_experts} active)"
                )
        else:
    ...
    for safetensor_file in list(unique_files)[:3]:  # Limit to first few files for speed
        try:
            ...
            response = requests.get(
                file_url, timeout=10, stream=True, headers={"Range": "bytes=0-8191"}
            )  # First 8KB should contain metadata
...
def calculate_model_size_from_safetensors(
    index_data: dict, model_name: str = ""
) -> tuple[int, str]:
    ...
    tensor_shapes = (
        fetch_tensor_shapes_from_safetensors(model_name, index_data) if model_name else {}
    )
...
def estimate_vram_requirements(
    model_name: str, safety_factor: float = 2.5, sequence_length: int = 2048, batch_size: int = 2
) -> dict:
    """
    ...
    if safetensors_info:
        params_from_weights, weights_breakdown = calculate_model_size_from_safetensors(
            safetensors_info, model_name
        )
    ...
    if "moe" in architecture.lower() or config.get("num_experts", 0) > 1:
        ...
        if experts_per_token and total_experts and experts_per_token < total_experts:
            ...
            expert_params_per_layer = experts_per_token * (
                hidden_size * intermediate_size + intermediate_size * hidden_size
            )
            router_params_per_layer = (
                hidden_size * total_experts
            )  # Router needs to know all experts

            # Total effective parameters
            embedding_params = vocab_size * hidden_size
            output_params = (
                vocab_size * hidden_size if not config.get("tie_word_embeddings", False) else 0
            )
            layer_params = num_layers * (
                non_expert_per_layer + expert_params_per_layer + router_params_per_layer
            )
            final_ln_params = hidden_size
            ...
            architecture += (
                f" (MoE: {experts_per_token}/{total_experts} experts active, effective params)"
            )
    ...
    dtype_to_bytes = {"float32": 4, "float16": 2, "bfloat16": 2, "int8": 1, "int4": 0.5}
    ...
    kv_cache_bytes = (
        2 * batch_size * num_heads * sequence_length * head_dim * num_layers * bytes_per_param
    )
    ...
    activation_bytes = (
        batch_size * sequence_length * hidden_size * num_layers * 4 * bytes_per_param
    )  # 4x hidden size for activations
...
def print_vram_estimate(estimate: dict):
    ...
    print(
        f"Total Parameters: {estimate['params_billions']:.1f}B ({estimate['estimated_params']:,})"
    )
    if (
        "effective_params" in estimate
        and estimate["effective_params"] != estimate["estimated_params"]
    ):
        print(
            f"Effective Parameters: {estimate['effective_params_billions']:.1f}B ({estimate['effective_params']:,})"
        )
        print(f"Effective Model Size: {estimate['effective_size_gb']:.1f}GB")
    print(f"Model Size: {estimate['model_size_gb']:.1f}GB")
    print(
        f"Native Precision: {estimate['torch_dtype']} ({estimate['bytes_per_param']} bytes/param)"
    )
    ...
    if "MoE:" in estimate["architecture"]:
        print(
            "  ⚡ MoE Note: Only active experts loaded, significant memory savings vs full parameter count"
        )
...
def main():
    ...
    parser.add_argument(
        "--safety-factor", type=float, default=2.5, help="Safety factor multiplier (default: 2.5)"
    )
    parser.add_argument(
        "--sequence-length", type=int, default=2048, help="Context length in tokens (default: 2048)"
    )
    ...
    estimate = estimate_vram_requirements(
        args.model, args.safety_factor, args.sequence_length, args.batch_size
    )
